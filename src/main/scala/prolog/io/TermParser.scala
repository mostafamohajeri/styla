...
class TermParser(val vars: LinkedHashMap[String, Var])
  ...
  def randomString(length: Int) = {
    s"var_${Random.alphanumeric take 10 mkString}"
  }

  def mkVar(x0: String) = {
    val x = if ("_" == x0) x0 + x0 + randomString(15) else x0
    vars.getOrElseUpdate(x, new Var(x0))
  }
...
object TermParser {
  ...
  def printclause(rvars: VMAP, xs: List[Term]) =
    IO.printlnInfo(clause2string(rvars, xs))
